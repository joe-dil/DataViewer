...
static int calculate_field_display_width_pure(const FieldDesc* field, const DSVConfig *config);
...
int analyze_column_widths(const char *data, size_t length,
    ...
    if (!data || !line_offsets || !result || !config) return -1;

    size_t sample_lines = num_lines > (size_t)config->column_analysis_sample_lines ? (size_t)config->column_analysis_sample_lines : num_lines;
    if (sample_lines == 0) {
        result->num_cols = 0;
    ...
    FieldDesc* fields = malloc(config->max_cols * sizeof(FieldDesc));
    if (!fields) {
        free(col_widths_tmp);
        return -1;
    }
    ...
    for (size_t i = 0; i < sample_lines; i++) {
        ...
        size_t num_fields = parse_line_pure(line_start, file_end, delimiter, fields, config->max_cols);
        ...
        for (size_t col = 0; col < num_fields && col < (size_t)config->max_cols; col++) {
            ...
            int width = calculate_field_display_width_pure(&fields[col], config);
...
static int calculate_field_display_width_pure(const FieldDesc *field, const DSVConfig *config) {
    ...
    char* temp_buffer = malloc(config->max_field_len);
    wchar_t* wide_buffer = malloc(config->max_field_len * sizeof(wchar_t));
    if (!temp_buffer || !wide_buffer) {
        free(temp_buffer);
        free(wide_buffer);
        return 0;
    }
    ...
    if (field->needs_unescaping) {
        render_field_pure(field, temp_buffer, config->max_field_len);
        mbstowcs(wide_buffer, temp_buffer, config->max_field_len);
        int display_width = wcswidth(wide_buffer, config->max_field_len);
        free(temp_buffer);
        free(wide_buffer);
        return display_width < 0 ? (int)strlen(temp_buffer) : display_width;
    } else {
        size_t copy_len = raw_len < (size_t)(config->max_field_len - 1) ? raw_len : (size_t)(config->max_field_len - 1);
        ...
        free(temp_buffer);
        free(wide_buffer);
        return display_width < 0 ? (int)copy_len : display_width;
    }
} 